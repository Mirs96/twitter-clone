...
// Function to get authentication headers for FormData
...
export const customFetch = async <T>(url: string, options: RequestInit = {}): Promise<T> => {
    // Determine which headers to use depending on the request body type
    const headers = options.body instanceof FormData ? getAuthHeadersFormData() : getAuthHeaders();
    
    // Perform the HTTP request
    const response = await fetch(`${HttpConfig.apiUrl}${url}`, {
        ...options,
        headers: {
            ...headers,
            ...options.headers,
